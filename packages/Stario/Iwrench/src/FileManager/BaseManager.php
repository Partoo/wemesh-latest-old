...
class BaseManager
{
    ...
    {
        return 'http://' . config('filesystems.disks.qiniu.domain') . '/' . ltrim($path, '/');
    ...
    public function store(UploadedFile $file, $dir = '', $filenameType = 'original')
    {
        ...
        $realname = $this->disk->putFileAs('upload/' . $dir, $file, $fileName);
        return response()->json([
            ...
            'relative_url' => "$realname",
            'url' => $this->fileWebPath("$realname"),
        ]);
    }
}
